...
A gutter draws a colored line down the left edge of every annotated line, in live preview and in source mode, where the text itself stays uncolored: red, green, both for a replacement, blue for a comment. The strip is drawn in the page margin and takes no room of its own, so the text keeps its place under the note title. The styling, the author display and the gutter are each a setting.
