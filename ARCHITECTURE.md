...
- **Percent marks are an ordinary wrapper for comments too.** `%%span%%^[reply]` is a comment on a hidden span, and the reply shows while the span stays hidden, which is the accepted cost of hiding it. A comment on a spot is only ever a `{>>note<<}`.
