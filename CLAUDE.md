...
**A bare highlight or comment is not an annotation.** `==text==` and `%%text%%` are ordinary Obsidian syntax and appear in notes that have nothing to do with annotations. They only count when an entry is attached. `{==text==}` does count on its own, because braces mean nothing else in Markdown. Losing this rule fills the sidebar with every highlight in the vault.
