...
**Obsidian's strikethrough and highlight classes sit on a parent span.** A text decoration or background on a parent cannot be undone from a child span, so the rules that remove the line from a `{~~replacement~~}` and the yellow from a `{==commented span==}` target the parent with `:has()`. A rule on our own span, however specific, never reaches them.
